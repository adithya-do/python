...
#    RIGHT -> "🗄️  Database Pulse" brand.
#    Clicking a button highlights it and routes to the correct view.
# 3) Landing page: "Database Pulse" in blue, nicer emojis, smaller subtitle.
#
...
class Landing(ttk.Frame):
    def __init__(self, master):
        ...
        icon = tk.Label(center, text="🗄️  ❤️‍🩹", font=("Segoe UI Emoji", 44))
        icon.pack(pady=(0,8))
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        ...
        tk.Label(right, text="🗄️  Database Pulse  ", bg="#f5a623", fg="#000000",
                 font=("Segoe UI", 14, "bold")).pack(side=tk.RIGHT)

        # Buttons (bold) with selection highlighting
        self._nav_btns: Dict[str, tk.Button] = {}
        def mkbtn(txt, key, cmd):
            b = tk.Button(left, text=txt, command=lambda k=key, c=cmd: self._select_nav(k, c),
                          bg="#ffffff", fg="#000000", activebackground="#ffe3ba",
                          activeforeground="#000000", font=("Segoe UI", 10, "bold"), padx=10, pady=4)
    ...
    def _set_window_icon(self):
        ...
        for x in range(16):
            for y in range(16):
                img.put("#ffffff", (x, y))
        # blue cylinder (rect body)
        for x in range(2, 14):
            for y in range(5, 13):
                img.put("#2b6cff", (x, y))
        # top ellipse band (lighter)
        for x in range(3, 13):
            img.put("#a7c3ff", (x, 5))
            img.put("#7fa9ff", (x, 6))
        # bottom highlight
    ...
    def _select_nav(self, key: str, route_call):
        ...
        if key in self._nav_btns:
            self._nav_btns[key].configure(bg="#ffd186")  # selected color
        route_call()
