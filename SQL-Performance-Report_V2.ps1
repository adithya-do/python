...
function ConvertTo-HtmlTable {
    ...
    $null = $sb.Append("<div class='table-wrapper'><table class='$CssClass'><thead><tr>")
    foreach ($col in $Table.Columns) {
        $null = $sb.Append("<th>$([System.Web.HttpUtility]::HtmlEncode($col.ColumnName))</th>")
    }
    ...
    foreach ($row in $Table.Rows) {
        ...
        foreach ($col in $Table.Columns) {
            ...
            $null = $sb.Append("<td>$([System.Web.HttpUtility]::HtmlEncode($val))</td>")
        }
        $null = $sb.Append("</tr>")
    }
...
# TempDB pressure
if ($tempDBUsage -and (Safe-Rows $tempDBUsage).Count -gt 0) {
    $maxTempKB = @(Safe-Rows $tempDBUsage | ForEach-Object {
...
$totalDBs     = if ($databases -and $databases.PSObject.Properties['Rows']) { $databases.Rows.Count } else { 0 }
...
:root{
    ...
    --border:#2e3350;--text:#e2e8f0;--text-muted:#8892b0;--text-dim:#4a5568;
    --accent:#6c63ff;--accent2:#00d9ff;--accent3:#ff6b9d;
    --green:#00e676;--yellow:#ffd740;--orange:#ff6d00;--red:#ff1744;
    --chart1:#6c63ff;--chart2:#00d9ff;--chart3:#ff6b9d;--chart4:#00e676;
    --chart5:#ffd740;--chart6:#ff6d00;--chart7:#a78bfa;--chart8:#34d399;
...
/* ── Layout ── */
.wrapper{max-width:1600px;margin:0 auto;padding:24px}

/* ── Header ── */
.report-header{
    background:linear-gradient(135deg,#1a1d27 0%,#22263a 50%,#2a2f45 100%);
...
.report-header h1{font-size:28px;font-weight:700;color:var(--text);margin-bottom:6px}
.report-header h1 span{color:var(--accent2)}
.report-header .meta{color:var(--text-muted);font-size:13px;display:flex;flex-wrap:wrap;gap:20px;margin-top:12px}
.report-header .meta span{display:flex;align-items:center;gap:6px}
.report-header .meta span::before{content:'';width:8px;height:8px;border-radius:50%;background:var(--accent)}
...
.kpi-label{font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:var(--text-muted);margin-bottom:8px}
...
.chart-card{background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:20px}
.chart-title{font-size:13px;font-weight:600;color:var(--text-muted);margin-bottom:16px;text-transform:uppercase;letter-spacing:.6px}
...
table.data-table td{
    padding:9px 14px;border-bottom:1px solid rgba(46,51,80,.5);
    color:var(--text);vertical-align:top;
}
table.data-table tr:last-child td{border-bottom:none}
table.data-table tr:hover td{background:rgba(108,99,255,.07)}
table.data-table td:first-child{font-family:var(--font-mono);font-size:12px}

/* SQL Text cells */
.sql-cell{font-family:var(--font-mono);font-size:11px;color:var(--accent2);
    max-width:400px;white-space:pre-wrap;word-break:break-all;background:rgba(0,217,255,.05);
    padding:6px 8px;border-radius:4px;border-left:2px solid var(--accent2)}

/* ── Recommendations ── */
.rec-card{border-radius:var(--radius-sm);margin-bottom:14px;overflow:hidden;border:1px solid var(--border)}
.rec-header{padding:12px 18px;display:flex;align-items:center;gap:10px;background:var(--surface2)}
.rec-category{font-size:11px;background:var(--surface3);padding:2px 8px;border-radius:20px;color:var(--text-muted)}
...
.rec-critical .rec-header{border-left:4px solid var(--red)}
.rec-high .rec-header{border-left:4px solid var(--orange)}
.rec-medium .rec-header{border-left:4px solid var(--yellow)}
.rec-low .rec-header{border-left:4px solid var(--green)}
.rec-good .rec-header{border-left:4px solid var(--green)}
...
.badge-critical{background:rgba(255,23,68,.2);color:#ff6b6b;border:1px solid rgba(255,23,68,.4)}
.badge-high{background:rgba(255,109,0,.2);color:#ffab40;border:1px solid rgba(255,109,0,.4)}
.badge-medium{background:rgba(255,215,64,.2);color:#ffd740;border:1px solid rgba(255,215,64,.4)}
.badge-low{background:rgba(0,230,118,.2);color:#69f0ae;border:1px solid rgba(0,230,118,.4)}
.badge-good{background:rgba(0,230,118,.2);color:#69f0ae;border:1px solid rgba(0,230,118,.4)}

/* ── Navigation ── */
.nav{
    position:sticky;top:0;z-index:100;background:rgba(15,17,23,.95);
...
.nav a:hover,.nav a.active{color:var(--accent2);border-bottom-color:var(--accent2)}
...
.footer{text-align:center;color:var(--text-dim);font-size:12px;padding:24px;margin-top:8px}
.scroll-top{
    ...
    box-shadow:0 4px 20px rgba(108,99,255,.5);transition:transform .2s;z-index:200;
}
...
# Dynamic KPI cards
function Safe-Rows { param($t) if ($t -and $t.PSObject.Properties['Rows']) { $t.Rows } else { @() } }

$physMem = if ($serverInfo -and (Safe-Rows $serverInfo).Count -gt 0) { "$((Safe-Rows $serverInfo)[0].PhysicalMemoryMB) MB" } else { "N/A" }
$logCPU  = if ($serverInfo -and (Safe-Rows $serverInfo).Count -gt 0) { (Safe-Rows $serverInfo)[0].LogicalCPUs } else { "N/A" }
$sqlUptime = if ($serverInfo -and (Safe-Rows $serverInfo).Count -gt 0) {
    try {
        $start = [datetime](Safe-Rows $serverInfo)[0].SQLStartTime
        $span  = (Get-Date) - $start
        "$([int]$span.TotalDays)d $($span.Hours)h $($span.Minutes)m"
    } catch { "N/A" }
} else { "N/A" }
$sqlMemUsed = if ($memoryInfo -and (Safe-Rows $memoryInfo).Count -gt 0) { "$((Safe-Rows $memoryInfo)[0].SQL_Physical_MB) MB" } else { "N/A" }
$bufferMB   = if ($bufferPool -and (Safe-Rows $bufferPool).Count -gt 0)  { "$((Safe-Rows $bufferPool)[0].TotalBufferMB) MB" }  else { "N/A" }
$missingIdxCount = (Safe-Rows $missingIdx).Count
$blockCount      = @(Safe-Rows $blocking | Where-Object { $_ -and $_.BlockedBy -ne 0 }).Count
$fragCount       = (Safe-Rows $fragmentation).Count
$waitTopStr      = if ($waitStats -and (Safe-Rows $waitStats).Count -gt 0) { (Safe-Rows $waitStats)[0].WaitType } else { "N/A" }
...
function makeBarChart(id, labels, data, label, color, horizontal) {
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { grid: { color: '#2e3350' }, ticks: { maxRotation: horizontal ? 0 : 45 } },
                y: { grid: { color: '#2e3350' } }
            }
        }
    });
...
function makeDoughnutChart(id, labels, data) {
    ...
    new Chart(ctx, {
        type: 'doughnut',
        data: { labels, datasets: [{ data, backgroundColor: COLORS, borderColor: '#1a1d27', borderWidth: 2, hoverOffset: 8 }] },
        options: {
            responsive: true, maintainAspectRatio: false,
            plugins: {
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: { grid: { color: '#2e3350' }, title: { display: true, text: 'ms', color: '#8892b0' } }
            }
        }
    });
}
