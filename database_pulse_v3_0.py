#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Database Pulse — Version 3.0
# Changes requested:
# - Restore the *same* leaf icon look-and-feel as V1 (neutral, no green fill),
#   and use it both for the window title bar and the banner.
# - Keep banner buttons highlight as medium blue, buttons not bold.
# - Show version text in top bar.
# - Landing page keeps 🗄️ + 📈 (no heart).
#
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        super().__init__()
        self.title(APP_NAME)
        # Use the *same* neutral leaf icon for title bar and banner (no green fill).
        self._leaf_small = self._make_leaf_icon_v1(16, 16)
        self.iconphoto(True, self._leaf_small)
        try:
        ...
        # LEFT: nav buttons
        left = tk.Frame(self.banner, bg="#f5a623")
        left.pack(side=tk.LEFT, padx=6, pady=6)

        # Buttons (normal weight) with selection highlighting (medium blue)
        ...
        # RIGHT: brand + version (with the *same* leaf icon as in title bar)
        right = tk.Frame(self.banner, bg="#f5a623")
        right.pack(side=tk.RIGHT, padx=10, pady=6)
        self._leaf_banner = self._make_leaf_icon_v1(18, 18)
        tk.Label(right, image=self._leaf_banner, bg="#f5a623").pack(side=tk.RIGHT, padx=(0,6))
        tk.Label(right, text=f"{APP_NAME}", bg="#f5a623", fg="#000000",
                 font=("Segoe UI", 14, "bold")).pack(side=tk.RIGHT)
        tk.Label(right, text=f" {APP_VERSION} ", bg="#f5a623", fg="#222222",
    ...
    # --- Leaf icon (neutral V1 style) ---
    def _make_leaf_icon_v1(self, w: int, h: int) -> tk.PhotoImage:
        """Draw a neutral leaf: light fill, dark outline, subtle vein. No green fill."""
        img = tk.PhotoImage(width=w, height=h)
        # Background transparent-like (use near white so it blends with most title bars)
        for x in range(w):
            for y in range(h):
                img.put("#ffffff", (x, y))
        # Leaf body (very light gray)
        for x in range(2, w-2):
            for y in range(3, h-3):
                nx = (x - w/2) / (w/2.2)
                ny = (y - h/2) / (h/2.8)
                if nx*nx + ny*ny <= 1.0 and y <= h - 4:
                    img.put("#f3f6f4", (x, y))
        # Outline
        for x in range(2, w-2):
            for y in range(3, h-3):
                ...
                ny = (y - h/2) / (h/2.8)
                d = nx*nx + ny*ny
                if 0.96 <= d <= 1.02 and y <= h - 4:
                    img.put("#3a3a3a", (x, y))
        # Vein (subtle diagonal)
        for i in range(3, min(w, h)-2):
            vx = int(i * (w-6) / (h-6)) + 3
            vy = h - i - 1
            if 2 <= vx < w-2 and 2 <= vy < h-2:
                img.put("#9aa3a0", (vx, vy))
        return img
    ...
    def _select_nav(self, key: str, route_call):
        ...
        if key in self._nav_btns:
            self._nav_btns[key].configure(bg="#6ea8fe")  # medium blue highlight
        route_call()
