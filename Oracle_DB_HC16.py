#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Oracle DB Health Monitor — Version 16
# - JDBC thin form now shows **User** and **Password**, and they are saved & used.
# - Service Name / SID selector and input row was not packed before; fixed (now visible).
# - Buttons row moved to the real bottom via an expanding spacer.
# - Still exactly two connection types: TNS (thick) and JDBC thin (EZCONNECT).
#
...
class DbEditor(tk.Toplevel):
    def __init__(self, parent: 'MonitorApp', target: Optional[DbTarget] = None, on_save=None):
        ...
        # make a vertical expander to push buttons to bottom
        frm.grid_columnconfigure(0, weight=1)

        # We'll use pack in blocks but add a final expanding spacer
        # DB Name
        ...
        # <-- this frame MUST be packed; previously it wasn't, causing it to be invisible
        ...
        # JDBC creds (now visible & saved/used)
        rr = ttk.Frame(self.row_jdbc); rr.pack(fill=tk.X, pady=4)
        ttk.Label(rr, text="User:", width=22).pack(side=tk.LEFT)
