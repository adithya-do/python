...
def logical_columns() -> List[str]:
    return [
        "S.No","DB Name","Environment","Host","DB Version","Startup Time","Status",
        "Inst_status","Sessions","WorstTS%","TS Online","DB Size","LastFull/Inc",
        "LastArch","Ms","LastChecked","Check status","Error",
    ]

FILTERABLE_COLUMNS = {"DB Name", "Environment", "Host", "DB Version", "Status"}
...
def connect_target(target: DbTarget, cfg: Dict[str, Any]):
    ...
    mode = (target.mode or "thin").lower(); dsn = (target.dsn or "").strip()
    user = (target.user or "").strip() or None
    pwd = _decrypt_password(target.password_enc) if target.password_enc else None
    if mode=="thick":
        init_oracle_client_if_needed(cfg); tns = normalize_tns(dsn)
        ...
        if not host: raise ValueError("Invalid JDBC thin DSN. Please set Host, Port and Service/SID.")
...
SQLS = {
    ...
    "tspace":("SELECT ts.tablespace_name, ROUND((1 - NVL(fs.free_mb,0)/ts.size_mb)*100,2) pct_used "
    ...
    "bk_data":("SELECT MAX(bp.completion_time) FROM v$backup_set bs JOIN v$backup_piece bp "
               "ON bs.set_stamp=bp.set_stamp AND bs.set_count=bp.set_count WHERE bs.backup_type='D'"),
    "bk_arch":("SELECT MAX(bp.completion_time) FROM v$backup_set bs JOIN v$backup_piece bp "
...
@dataclass
class _RowResult:
    status_cell: str = "-"; inst_cell: str = "-"; sessions_cell: str = "-"; worst_cell: str = "-"
    last_full_cell: str = "-"; last_arch_cell: str = "-"; version: str = "-"; startup_str: str = "-"
    ts_cell: str = "-"; db_size_cell: str = "-"; ms: int = 0; last_checked: str = "-"; check_status: str = "-"; error: str = ""
...
def check_one(target: DbTarget, cfg: Dict[str, Any], timeout_sec: int = 25) -> "DbHealth":
    ...
    try:
        with connect_target(target,cfg) as conn:
            ...
            try: cur.execute(SQLS["ts_online"]); total,tonline = cur.fetchone(); ts_total=int(total or 0); ts_online=int(tonline or 0)
            except Exception: pass
            db_size_gb=None
            try: cur.execute(SQLS["db_size"]); row=cur.fetchone(); db_size_gb=float(row[0]) if row and row[0] is not None else None
            except Exception: pass
...
class MonitorApp(ttk.Frame):
    ...
    def __init__(self, master, cfg: Dict[str, Any]):
        ...
        # Header filters: {col: set(values)}; None => no filter
    ...
    def _build_ui(self):
        ...
        style.configure("App.Treeview.Heading", background="#cfe8ff", foreground="#000", font=(self._font.actual("family"), self._font.actual("size"), "bold"))
        ...
        # Column customization + filter + client dir
        ...
        # Right-click behavior (rows vs header)
        ...
        self.tree.bind("<Button-3>", self._on_button3)  # route based on region
    ...
    def _row_passes_advanced(self, values: List[Any]) -> bool:
        ...
        def cmp_text(op: str, hay: str, needle: str) -> bool:
            ...
            try:
                ...
                if any(map(lambda x: x!=x, [hv, nv])): return False
    ...
    def _row_passes_header(self, values: List[Any]) -> bool:
        if not self._header_filters: return True
        colidx = {c:i for i,c in enumerate(self.LOGICAL_COLUMNS)}
    ...
    def _apply_all_filters(self):
        ...
        for iid in list(self.tree.get_children("")):
            ...
            if visible:
                ...
                self.tree.detach(iid); self._detached.add(iid)
    ...
    def _customize_columns(self):
        ...
        def apply_and_close():
            ...
            self.tree["displaycolumns"]=order; self.cfg["visible_columns"]=order; self._persist_column_layout(); dlg.destroy()
    ...
    def _select_columns_dialog(self):
        dlg = tk.Toplevel(self); dlg.title("Select Visible Columns"); dlg.geometry("360x520")
        ...
        def set_all(state: bool):
            for col,var in vars_by_col.items(): var.set(state)
        ...
        for col in self.LOGICAL_COLUMNS:
            ...
            v = tk.BooleanVar(value=(col in current_visible)); vars_by_col[col]=v
        ...
        def apply_and_close():
            ...
            if not visible: visible = ["S.No"]
            self.tree["displaycolumns"]=visible; self.cfg["visible_columns"]=visible; save_config(self.cfg)
            try: self._autosize_columns()
            except: pass
            dlg.destroy()
    ...
    def _select_email_columns_dialog(self):
        ...
        helper = ttk.Frame(dlg); helper.pack(fill=tk.X,padx=10,pady=(0,6))
        def set_all(state: bool):
            for v in vars_by_col.values(): v.set(state)
        ttk.Button(helper,text="Select All",command=lambda: set_all(True)).pack(side=tk.LEFT,padx=(0,6))
        ttk.Button(helper,text="Clear All",command=lambda: set_all(False)).pack(side=tk.LEFT)
        for col in all_cols:
            v = tk.BooleanVar(value=(col in current_email_cols)); vars_by_col[col]=v
            ttk.Checkbutton(box,text=col,variable=v).pack(anchor="w")
        def apply_and_close():
            current_display = list(self.tree["displaycolumns"]) or list(self.LOGICAL_COLUMNS)
            order_ref = current_display
            selected = [c for c in order_ref if vars_by_col.get(c, tk.BooleanVar(value=True)).get() and c in all_cols]
            if not selected: selected = [c for c in ["S.No","DB Name"] if c in all_cols]
            self.cfg["email_columns"]=selected; save_config(self.cfg); messagebox.showinfo(APP_NAME,f"Email columns updated ({len(selected)})")
            dlg.destroy()
        ttk.Button(dlg,text="Apply",command=apply_and_close).pack(pady=8)
    ...
    def _open_filter_dialog(self):
        ...
        val_var = tk.StringVar(); ttk.Entry(dlg,textvariable=val_var,width=20).grid(row=1,column=3,sticky="w",**pad)
    ...
    def _clear_filter(self):
        ...
        self.cfg["active_filter"]=[]; self.cfg["header_filters"]={}; save_config(self.cfg)
        self._refresh_heading_labels(); self._apply_all_filters()
    ...
    def _copy_cell(self):
        ...
        vals = self.tree.item(row)["values"]; text = str(vals[col_index]) if col_index < len(vals) else ""
    ...
    def _copy_by_col(self, colname: str):
        ...
        idx = self.LOGICAL_COLUMNS.index(colname); text = str(vals[idx]) if idx < len(vals) else ""
        self.clipboard_clear(); self.clipboard_append(text)
    ...
    def _apply_persisted_row(self, name: str, hdict: Dict[str, Any]):
        ...
        sc=int(hdict.get("sessions_curr",0)); sl=int(hdict.get("sessions_limit",0) or 0); sess_ok=(sl==0) or (sc < 0.95*sl)
        ...
        on=int(hdict.get("ts_online",0) or 0); tot=int(hdict.get("ts_total",0) or 0); ts_ok=(tot==on and tot>0)
        ...
        vals[colidx["Sessions"]]=sessions_cell; vals[colidx["WorstTS%"]]=worst_cell; vals[colidx["LastFull/Inc"]]=hdict.get("last_full_inc_backup_str",f"{BAD} -")
        vals[colidx["LastArch"]]=hdict.get("last_arch_backup_str",f"{BAD} -"); vals[colidx["DB Version"]]=hdict.get("version","-")
        vals[colidx["Startup Time"]]=startup_str; vals[colidx["TS Online"]]=ts_cell; vals[colidx["DB Size"]]=db_size_cell
    ...
    def _set_check_status(self, name: str, status: str):
        if name in self.tree.get_children("") or name in self._detached:
            if name in self._detached: self.tree.move(name,"","end"); self._detached.discard(name)
    ...
    def _apply_result(self, name: str, target: DbTarget, h: DbHealth):
        ...
        vals[colidx["Sessions"]]=sessions_cell; vals[colidx["WorstTS%"]]=worst_cell; vals[colidx["LastFull/Inc"]]=last_full_cell
        vals[colidx["LastArch"]]=last_arch_cell; vals[colidx["DB Version"]]=h.version or "-"
        vals[colidx["Startup Time"]]=startup_str; vals[colidx["TS Online"]]=ts_cell; vals[colidx["DB Size"]]=db_size_cell
        vals[colidx["Ms"]]=h.elapsed_ms; vals[colidx["LastChecked"]]=h.ts; vals[colidx["Check status"]]="Complete"
        vals[colidx["Error"]]=h.error or ("" if h.status=="UP" else h.details)
        self.tree.item(name,values=vals)
        self.last_health[name]={"status":h.status,"inst_status":h.inst_status,"sessions_curr":h.sessions_curr,"sessions_limit":h.sessions_limit,
                                "worst_ts_pct_used":h.worst_ts_pct_used,"host":h.host,"elapsed_ms":h.elapsed_ms,"version":h.version,
                                "startup_time_str":startup_str,"ts_online":h.ts_online,"ts_total":h.ts_total,"db_size_gb":h.db_size_gb,
                                "ts":h.ts,"error":vals[colidx["Error"]],"last_full_inc_backup_str":last_full_cell,"last_arch_backup_str":last_arch_cell}
        self.cfg["last_health"]=self.last_health; save_config(self.cfg)
        self._renumber(); self._autosize_columns(); self._apply_all_filters()
    ...
    def _clear_all_rows(self):
        ...
        self.status_var.set("Cleared all rows (except S.No, DB Name, Environment)."); self._apply_all_filters()
    ...
    def _clear_selected_row(self):
        ...
        if iid in self._detached: self.tree.move(iid,"","end"); self._detached.discard(iid)
        vals=list(self.tree.item(iid)["values"]); cleared=self._clear_row_values(vals); self.tree.item(iid,values=cleared)
        self.status_var.set(f"Cleared row: {iid}"); self._apply_all_filters()
    ...
    def _remove_selected(self):
        ...
        self._detached.discard(name); self.tree.delete(name); self._persist_targets(); self._renumber()
    ...
    def _update_target(self, t: DbTarget):
        ...
        if t.name in self.tree.get_children("") or t.name in self._detached:
            if t.name in self._detached: self.tree.move(t.name,"","end"); self._detached.discard(t.name)
    ...
    def _import_json(self):
        ...
        try:
            ...
            save_config(self.cfg)
            self._refresh_table_from_targets(); self._load_last_health_into_rows(); self._refresh_heading_labels(); self._apply_all_filters()
    ...
    def _export_json(self):
        ...
        try:
            export={"interval_sec":self.interval_var.get(),
                    ...
                    "email":{"server":self.smtp_server_var.get().strip(),"port":int(self.smtp_port_var.get() or 25),
                             "from_addr":self.from_var.get().strip(),"to_addrs":self.to_var.get().strip(),
...
class DbEditor(tk.Toplevel):
    def __init__(self, app: MonitorApp, target: Optional[DbTarget] = None, on_save=None):
        super().__init__(app)
        self.app=app; self.cfg=app.cfg; self.on_save=on_save
        self.title("Add / Edit Database"); self.resizable(False,False)
        ...
        if target and target.mode=="thin":
            h,p,svc,sid = parse_ezconnect(target.dsn); host,port=h or "", p or "1521"
        self.thin_host=tk.StringVar(value=host); self.thin_port=tk.StringVar(value=port or "1521")
        self.thin_conn_using=tk.StringVar(value=("Service Name" if svc else "SID" if sid else "Service Name"))
        self.thin_service=tk.StringVar(value=svc or ""); self.thin_sid=tk.StringVar(value=sid or "")
        ...
        ttk.Combobox(body,textvariable=self.env_var,values=["NON-PROD","PROD"],width=12,state="readonly").grid(row=row,column=1,sticky="w",padx=4,pady=4)
        row+=1
        ttk.Label(body,text="Connection Type:").grid(row=row,column=0,sticky="e",padx=4,pady=4)
        ttk.Radiobutton(body,text="TNS (Oracle Client)",variable=self.mode_var,value="thick",command=self._refresh_mode).grid(row=row,column=1,sticky="w")
        ttk.Radiobutton(body,text="JDBC Thin (no client)",variable=self.mode_var,value="thin",command=self._refresh_mode).grid(row=row,column=2,sticky="w")

        row+=1
        self.tns_block=ttk.LabelFrame(body,text="TNS (uses Oracle Client)"); self.tns_block.grid(row=row,column=0,columnspan=4,sticky="ew",padx=2,pady=6)
        ...
        row+=1
        self.thin_block=ttk.LabelFrame(body,text="JDBC Thin (no client)"); self.thin_block.grid(row=row,column=0,columnspan=4,sticky="ew",padx=2,pady=6)
        ...
        ttk.Radiobutton(self.thin_block,text="Service Name",variable=self.thin_conn_using,value="Service Name",command=self._refresh_mode).grid(row=1,column=1,sticky="w")
        ttk.Radiobutton(self.thin_block,text="SID",variable=self.thin_conn_using,value="SID",command=self._refresh_mode).grid(row=1,column=2,sticky="w")
        ttk.Label(self.thin_block,text="Service Name:").grid(row=2,column=0,sticky="e",padx=4,pady=4)
        self.entry_service=ttk.Entry(self.thin_block,textvariable=self.thin_service,width=24); self.entry_service.grid(row=2,column=1,sticky="w",padx=4,pady=4)
        ttk.Label(self.thin_block,text="SID:").grid(row=2,column=2,sticky="e",padx=4,pady=4)
        self.entry_sid=ttk.Entry(self.thin_block,textvariable=self.thin_sid,width=18); self.entry_sid.grid(row=2,column=3,sticky="w",padx=4,pady=4)
        ...
        self._refresh_mode(); self.grab_set(); self.transient(app)

    def _refresh_mode(self):
        thick=self.mode_var.get()=="thick"
        for child in self.tns_block.winfo_children(): child.configure(state=("normal" if thick else "disabled"))
        for child in self.thin_block.winfo_children(): child.configure(state=("normal" if not thick else "disabled"))
    ...
    def _test_connection(self):
        try:
            ...
            with connect_target(t,self.cfg) as conn:
                cur=conn.cursor(); cur.execute("select 1 from dual"); _=cur.fetchone()
    ...
    def _target_from_fields(self) -> DbTarget:
        name=self.name_var.get().strip(); env=self.env_var.get().strip() or "NON-PROD"; mode=self.mode_var.get()
        user=self.user_var.get().strip() or None; pwd=self.pass_var.get()
        if mode=="thick":
            ...
            host=self.thin_host.get().strip(); port=self.thin_port.get().strip() or "1521"
            if self.thin_conn_using.get()=="Service Name":
                service=self.thin_service.get().strip(); dsn=ezconnect_service(host,port,service)
            else:
                sid=self.thin_sid.get().strip(); dsn=ezconnect_sid(host,port,sid)
...
OracleMonitorApp = MonitorApp
OraclePlaceholder = MonitorApp
