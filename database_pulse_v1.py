...
# -*- coding: utf-8 -*-
"""
Database Pulse — Version 1 (fallback)
- Title bar: "Database Pulse"
- Top banner: medium orange with two left buttons: "Oracle Database" and "SQL Server"
- Landing page: centered "Database Pulse" title with small subtitle
- Oracle monitor: full-featured table, sorting, scrollbars, email report, config persistence
- Connection types: 1) TNS using Oracle Client (thick)  2) JDBC Thin (no client)
- Add/Edit DB dialog includes Test Connection; saves to config
- Config file location: ./config/oracle_config.json next to the script (created if missing)

Note: Requires python-oracledb for DB connectivity; app will run without it but checks will fail.
"""
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        super().__init__()
        self.title(APP_NAME)  # V1: plain title
        try:
            if sys.platform.startswith("win"):
        ...
        def mkbtn(txt, key, cmd):
            b = tk.Button(
                left,
                text=txt,
                command=lambda k=key, c=cmd: self._select_nav(k, c),
                bg="#ffffff",
                fg="#000000",
