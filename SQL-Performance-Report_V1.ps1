...
"@
$fragmentation = Invoke-SqlQuery -Query $qFragmentation
...
SELECT TOP 20
    ...
    t.task_user_objects_alloc_page_count * 8  AS TaskUserObjectsKB,
    r.total_elapsed_time / 1000         AS ElapsedSec,
    SUBSTRING(qt.text,(r.statement_start_offset/2)+1,
        ((CASE r.statement_end_offset WHEN -1 THEN DATALENGTH(qt.text)
          ELSE r.statement_end_offset END - r.statement_start_offset)/2)+1) AS StatementText
...
# TempDB pressure
if ($tempDBUsage -and $tempDBUsage.Rows.Count -gt 0) {
    $maxTempKB = ($tempDBUsage.Rows | ForEach-Object {
        [long]$_.InternalObjectsKB + [long]$_.UserObjectsKB
...
$totalDBs     = if ($databases) { $databases.Rows.Count } else { 0 }
...
# Dynamic KPI cards
$physMem = if ($serverInfo -and $serverInfo.Rows.Count -gt 0) { "$($serverInfo.Rows[0].PhysicalMemoryMB) MB" } else { "N/A" }
$logCPU  = if ($serverInfo -and $serverInfo.Rows.Count -gt 0) { $serverInfo.Rows[0].LogicalCPUs } else { "N/A" }
$sqlUptime = if ($serverInfo -and $serverInfo.Rows.Count -gt 0) {
    $start = [datetime]$serverInfo.Rows[0].SQLStartTime
    $span = (Get-Date) - $start
    "$([int]$span.TotalDays)d $($span.Hours)h $($span.Minutes)m"
} else { "N/A" }
$sqlMemUsed = if ($memoryInfo -and $memoryInfo.Rows.Count -gt 0) { "$($memoryInfo.Rows[0].SQL_Physical_MB) MB" } else { "N/A" }
$bufferMB   = if ($bufferPool -and $bufferPool.Rows.Count -gt 0) { "$($bufferPool.Rows[0].TotalBufferMB) MB" } else { "N/A" }
$missingIdxCount = if ($missingIdx) { $missingIdx.Rows.Count } else { 0 }
$blockCount = if ($blocking) { ($blocking.Rows | Where-Object { $_.BlockedBy -ne 0 } | Measure-Object).Count } else { 0 }
$fragCount = if ($fragmentation) { $fragmentation.Rows.Count } else { 0 }
$waitTopStr = if ($waitStats -and $waitStats.Rows.Count -gt 0) { $waitStats.Rows[0].WaitType } else { "N/A" }
