#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Oracle DB Health Monitor — Version 12 (JDBC thin fix)
# (see chat message for details)
...
def ezconnect_service(host: str, port: str, service: str) -> str:
    return f"//{host.strip()}:{(port or '1521').strip()}/{service.strip()}"

def ezconnect_sid(host: str, port: str, sid: str) -> str:
    return f"//{host.strip()}:{(port or '1521').strip()}/?sid={sid.strip()}"

def looks_like_ezconnect(dsn: str) -> bool:
    s = (dsn or "").lower().strip()
    return ("/" in s) or ("?sid=" in s) or (":" in s and "/" in s)
...
def default_config() -> Dict[str, Any]:
    ...
    return {"interval_sec": 300,"targets": [],"client_lib_dir": os.environ.get("ORACLE_CLIENT_LIB_DIR",""),"email": {"server": "", "port": 25, "from_addr": "", "to_addrs": "", "subject": "Oracle DB Health Report"},"last_health": {},"auto_run": False,"column_order": cols[:],"visible_columns": cols[:],"email_columns": cols[:],"column_widths": {}}
...
def _hydrate_target(d: Dict[str, Any]) -> DbTarget:
    ...
    return DbTarget(name=d.get("name",""), dsn=d.get("dsn","").strip(), user=d.get("user"), password=_decrypt_password(penc) if penc else None, password_enc=penc, wallet_dir=d.get("wallet_dir"), mode=d.get("mode","thin"), environment=d.get("environment","NON-PROD"))

def _serialize_target(t: DbTarget) -> Dict[str, Any]:
    enc = t.password_enc or (_encrypt_password(t.password) if t.password else None)
    return {"name": t.name, "dsn": t.dsn.strip(), "user": t.user, "password_enc": enc, "wallet_dir": t.wallet_dir, "mode": t.mode, "environment": t.environment}
...
def connect_target(target: DbTarget, cfg: Dict[str, Any]):
    ...
    mode = (target.mode or "thin").lower(); dsn = target.dsn.strip()
    if mode == "thick":
        init_oracle_client_if_needed(cfg)
        tns = normalize_tns(dsn)
        return try_connect(target.user, target.password or _decrypt_password(target.password_enc or ""), tns, target.wallet_dir)
    else:
        if not looks_like_ezconnect(dsn):
            raise ValueError("JDBC thin requires EZCONNECT DSN (e.g., //host:port/service or //host:port/?sid=SID).")
        return try_connect(target.user, target.password or _decrypt_password(target.password_enc or ""), dsn, None)
...
class DbEditor(tk.Toplevel):
    def __init__(self, parent: 'MonitorApp', target: Optional[DbTarget] = None, on_save=None):
        ...
        self.var_dsn = tk.StringVar(value=target.dsn if (target and target.mode=="thick") else "")
    ...
    def _build_thin_dsn(self) -> Optional[str]:
        host = self.var_host.get().strip(); port = (self.var_port.get().strip() or "1521")
        if self.var_use_service.get():
            service = self.var_service.get().strip()
            if not (host and port and service): return None
            return ezconnect_service(host, port, service)
        else:
            sid = self.var_sid.get().strip()
            if not (host and port and sid): return None
            return ezconnect_sid(host, port, sid)

    def _test_connection(self):
        mode = "thick" if self.var_conn_type.get() == "tns" else "thin"
        ...
        try:
            if mode == "thick":
                ...
                dsn = normalize_tns(self.var_dsn.get().strip())
                if not dsn:
                    messagebox.showerror(APP_NAME, "Enter a TNS alias/descriptor."); return
                if wallet:
                ...
                dsn = self._build_thin_dsn()
                if not dsn:
                    messagebox.showerror(APP_NAME, "Enter Host/Port and Service (or SID) for JDBC thin."); return
                if wallet:
                    conn = oracledb.connect(config_dir=wallet, dsn=dsn, user=user, password=pwd)
                elif user and pwd:
                    conn = oracledb.connect(user=user, password=pwd, dsn=dsn)
                else:
                    conn = oracledb.connect(dsn=dsn)
