#!/usr/bin/env python3
# Oracle DB Health GUI Monitor — Version 5
# See notes at bottom for usage.
...
SQLS = {
    ...
    "ts_online": "SELECT COUNT(*) total, SUM(CASE WHEN status='ONLINE' THEN 1 ELSE 0 END) online FROM dba_tablespaces",
...
class MonitorApp(ttk.Frame):
    COLUMNS = (
        ...
        "WorstTS%", "LastFull/Inc", "LastArch", "DB Version", "Startup Time", "TS Online",
        "DB Size", "Ms", "LastChecked", "Check status", "Error"
    )
    ...
    def _build_ui(self):
        ...
        col_widths = {
            "DB Version": 260, "Error": 360, "LastFull/Inc": 170, "LastArch": 170,
            "Environment": 120, "Host": 170, "DB Name": 170, "Check status": 120,
            "Startup Time": 170, "TS Online": 120, "DB Size": 110
        }
    ...
    def _apply_persisted_row(self, name: str, hdict: Dict[str, Any]):
        ...
        vals[11] = startup_str
        vals[12] = ts_cell
        vals[13] = db_size_cell
        vals[14] = hdict.get("elapsed_ms",0)
        vals[15] = hdict.get("ts","-")
        vals[16] = "Complete"
        vals[17] = hdict.get("error","")
    ...
    def _set_check_status(self, name: str, status: str):
        if name in self.tree.get_children():
            vals = list(self.tree.item(name)["values"])
            idx = self.COLUMNS.index("Check status")
            if len(vals) <= idx:
                vals += [""] * (idx + 1 - len(vals))
            vals[idx] = status
            self.tree.item(name, values=vals)
    ...
    def _apply_result(self, name: str, target: DbTarget, h: DbHealth):
        ...
        vals = list(self.tree.item(name)["values"] or ["-"]*len(self.COLUMNS))
        def set_col(col, value):
            idx = self.COLUMNS.index(col)
            if len(vals) <= idx:
                vals.extend([""]*(idx+1-len(vals)))
            vals[idx] = value

        set_col("Host", h.host or "-")
        set_col("Status", status_cell)
        set_col("Inst_status", inst_cell)
        set_col("Sessions", sessions_cell)
        set_col("WorstTS%", worst_cell)
        set_col("LastFull/Inc", last_full_cell)
        set_col("LastArch", last_arch_cell)
        set_col("DB Version", h.version or "-")
        set_col("Startup Time", startup_str)
        set_col("TS Online", ts_cell)
        set_col("DB Size", db_size_cell)
        set_col("Ms", h.elapsed_ms)
        set_col("LastChecked", h.ts)
        set_col("Check status", "Complete")
        set_col("Error", h.error or ("" if h.status == "UP" else h.details))
        ...
        self.last_health[name] = {
            ...
            "ts": h.ts,
            "error": vals[self.COLUMNS.index("Error")],
