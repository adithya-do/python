#!/usr/bin/env python3
"""
Oracle DB Health Monitor — Version 12

Changes from v11:
1) Header banner: light orange background with bold black title text.
2) Config file renamed to "oracle_config.json".
3) "Email Columns" button moved next to "Email Report".
4) JDBC thin DSN fixed: service uses "host:port/service" (no leading //); SID uses "host:port:SID".
"""
...
CONFIG_PATH = CONFIG_DIR / "oracle_config.json"  # updated name
...
class MonitorApp(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Header banner
        header = tk.Frame(self, bg="#ffd8a8")
        header.pack(side=tk.TOP, fill=tk.X)
        title = tk.Label(header, text=APP_NAME, bg="#ffd8a8", fg="#000000",
                         font=(self._font.actual('family'), self._font.actual('size')+2, "bold"), padx=10, pady=6)
    ...
    def _build_dsn(self) -> str:
        if self.var_conn_type.get() == "jdbc":
            ...
            if self.var_use_service.get():
                ...
                return f"{host}:{port}/{service}"   # fixed
            else:
