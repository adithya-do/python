#!/usr/bin/env python3
"""
Oracle DB Health GUI Monitor — Version 11

Fixes over v10:
1) Tree header styling: force 'clam' theme and light blue (#cfe8ff) header background with black bold font.
2) Add/Edit DB: connection type choice: "TNS / Oracle Client" or "JDBC thin".
   - For JDBC thin, the dialog shows Host, Port, and SID / Service Name toggle with corresponding input.
   - DSN is auto-built for python-oracledb (EZConnect) as:
       //host:port/service_name   (service name)
       host:port:SID              (SID)

Keeps:
- Column visibility/order persistence, sortable headers, right-click copy.
- Email report with selectable columns.
- Windows DPAPI encryption (fallback base64) for saved passwords.
- Persisted last health results and layout.
"""
...
CONFIG_PATH = CONFIG_DIR / "oracle_config.jason"  # name per prior request
...
class MonitorApp(ttk.Frame):
    ...
    STATUS_COLUMNS = {
        # Columns we clear with "Clear" buttons (preserve S.No, DB Name, Environment)
    ...
    def _build_ui(self):
        ...
        try:
            style.theme_use("clam")  # allows heading bg color
        except Exception:
            pass
...
# ----- DB Editor with connection choice -----
class DbEditor(tk.Toplevel):
    def __init__(self, parent: MonitorApp, target: Optional[DbTarget] = None, on_save=None):
        ...
        # Connection type
        self.var_conn_type = tk.StringVar(value="tns")
        if target and target.dsn and (":" in target.dsn or "/" in target.dsn) and not target.dsn.strip().upper().startswith("("):
            # Heuristic: looks like EZConnect/JDBC => default to tns unless user chooses jdbc
            self.var_conn_type.set("tns")
        # TNS field
        ...
        ttk.Label(self.row_tns, text="TNS Alias / EZConnect:", width=22).pack(side=tk.LEFT)
    ...
    def _toggle_conn_type(self):
        ...
        if tns:
            ...
            # The specific SID/Service row is toggled below
    ...
    def _build_dsn(self) -> str:
        ...
        # TNS/EZConnect field directly
        return self.var_dsn.get().strip()

    def _test_connection(self):
        dsn_in = self._build_dsn()
    ...
    def _save(self):
        ...
        if not name:
            ...
            return
        dsn = normalize_dsn(self._build_dsn())
        pwd = self.var_pwd.get().strip() or None
