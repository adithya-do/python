#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# === Stable Base: Database Pulse (Oracle) — v18 pulse fixed ===

"""
This build merges your stable Oracle monitor (v18 lineage) with a router UI:
- Title bar: "Database Pulse"
- Top banner (light orange) with left buttons: Home, Oracle Database, SQL Server (future)
- App opens on Landing; Oracle shows your monitor in a dedicated container (no overlap)
- Uses grid/tkraise to swap views cleanly (no pack/grid conflicts)
- Keeps v18 features: TNS vs JDBC thin, Test Connection, table, email HTML, persistence

If python-oracledb isn't installed, the GUI still opens; connection attempts will complain.
"""
...
class RouterApp(tk.Tk):
    ...
    def show_oracle(self):
        # Use a dedicated container inside the stack; grid MonitorApp within it
    ...
    def show_sqlserver(self):
        if self.view_sql is None:
            ...
            if not self.sql_container.winfo_children():
                ...
                tk.Label(inner, text="SQL Server Monitoring — Coming Soon", font=("Segoe UI", 20, "bold")).grid(sticky="nsew")
        try:
