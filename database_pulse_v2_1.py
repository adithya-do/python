#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Database Pulse — Version 2.1
# Improvements:
# 1) Leaf icon used for the window title bar and also shown on the top banner before "Database Pulse".
# 2) Top banner LEFT buttons order: Home, Oracle Database, SQL Server (normal weight). Selected button = medium blue.
# 3) Top banner RIGHT shows leaf icon + "Database Pulse" + small version text.
# 4) Landing page uses 🗄️ + 📈 (no heart), blue title, smaller subtitle.
#
...
class Landing(ttk.Frame):
    def __init__(self, master):
        ...
        # Pretty landing with blue title & smaller subtitle
        icon = tk.Label(center, text="🗄️  📈", font=("Segoe UI Emoji", 44))  # no heart emoji
        icon.pack(pady=(0,8))
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Window chrome
        self.title(APP_NAME)
        self._leaf_img_small = self._make_leaf_icon(16, 16)   # for title bar
        self.iconphoto(True, self._leaf_img_small)
        try:
        ...
        # Leaf icon (slightly larger for banner)
        self._leaf_img_banner = self._make_leaf_icon(18, 18)
        tk.Label(right, image=self._leaf_img_banner, bg="#f5a623").pack(side=tk.RIGHT, padx=(0,6))
    ...
    # --- Leaf icon drawing ---
    def _make_leaf_icon(self, w: int, h: int) -> tk.PhotoImage:
        img = tk.PhotoImage(width=w, height=h)
        # Fill white
        for x in range(w):
            for y in range(h):
                img.put("#ffffff", (x, y))
        # Simple green "leaf" (oval-ish) with darker vein
