...
BEGIN
    ...
    BEGIN
        ...
        -- -----------------------------------------------------------------------
        -- INSERTs: rows in remote source NOT in local target
        -- -----------------------------------------------------------------------
        v_diff_sql :=
            'SELECT ''INSERT'', ' || v_pk_sel_s
            || ' FROM ' || v_src_full || ' s'          -- remote source
            || ' WHERE NOT EXISTS ('
            ||     'SELECT 1 FROM ' || v_tgt_full || ' t'   -- local target
            ||     ' WHERE ' || v_pk_join
            || ')'

        -- -----------------------------------------------------------------------
        -- DELETEs: rows in local target NOT in remote source
        -- -----------------------------------------------------------------------
            || ' UNION ALL '
            || 'SELECT ''DELETE'', ' || v_pk_sel_t
            || ' FROM ' || v_tgt_full || ' t'          -- local target
            || ' WHERE NOT EXISTS ('
            ||     'SELECT 1 FROM ' || v_src_full || ' s'   -- remote source
            ||     ' WHERE ' || v_pk_join
            || ')';

        -- -----------------------------------------------------------------------
        -- UPDATEs: rows in both with at least one changed data column
        -- -----------------------------------------------------------------------
        IF v_data_cols.COUNT > 0 THEN
            v_diff_sql := v_diff_sql
                ...
                || ' FROM ' || v_src_full || ' s'      -- remote source
                || ' JOIN ' || v_tgt_full || ' t'      -- local target
                ||     ' ON (' || v_pk_join || ')'
