...
# -*- coding: utf-8 -*-
"""
Oracle DB Health Monitor — Version 13

Changes:
- Custom title bar (light orange bg, bold black text) replaces native bar look.
- Strict two connection types:
  1) TNS / Oracle Client (thick; expects TNS alias/descriptor; uses client lib)
  2) JDBC thin (EZConnect; no client; service or SID)
- JDBC thin DSN build & fallbacks; Test Connection = runtime logic.
"""
...
# ---------- JDBC thin -> EZConnect helpers ----------
def normalize_dsn(dsn: str) -> str:
    """Strip common JDBC/at-sign prefixes and a leading '//'."""
    s = (dsn or "").strip()
    lower = s.lower()
...
def build_ezconnect(host: str, port: str, service: Optional[str], sid: Optional[str]) -> List[str]:
    """Return a list of candidate EZConnect strings to try."""
    host = (host or "").strip()
    ...
    if service:
        ...
        if not svc:
            return []
        # Primary + fallbacks
        candidates.append(f"{host}:{port}/{svc}")
        candidates.append(f"//{host}:{port}/{svc}")
        candidates.append(f"{host}/{svc}")  # portless fallback
    elif sid:
        s = sid.strip()
        if not s:
            return []
        # EC Plus preferred, then legacy SID
        candidates.append(f"{host}:{port}/?sid={s}")
        candidates.append(f"{host}:{port}:{s}")
        candidates.append(f"//{host}:{port}:{s}")
    return candidates
...
class DbTarget:
    ...
    mode: str = "thin"       # "thick" for TNS alias, "thin" for JDBC/EZConnect
...
def try_connect(user, pwd, dsn, wallet_dir=None):
    """Attempt connect with given parameters; return connection or raise."""
    if wallet_dir:
...
def _connect(target: DbTarget):
    ...
    # Initialize Thick only if TNS mode is used
    ...
    try:
        ...
        # If TNS resolution error, retry with // prefix
...
class MonitorApp(ttk.Frame):
    ...
    # ----- Custom title bar (light orange, bold black) -----
    def _install_titlebar(self, root: tk.Tk):
        # Native titlebar colors can't be changed; implement custom draggable bar.
    ...
    def _build_ui(self):
        ...
        # Apply persisted display order and visibility
...
class DbEditor(tk.Toplevel):
    ...
    def _toggle_conn_type(self):
        ...
        if tns:
            self.var_mode.set("thick")  # force thick for TNS
            ...
            self.var_mode.set("thin")  # force thin for JDBC
    ...
    def _build_dsn_candidates(self) -> List[str]:
        ...
        # TNS path: use whatever user typed (TNS alias or descriptor/EZConnect)
