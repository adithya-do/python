#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# (short header here to avoid tool timeouts; full docstring omitted)
# Oracle DB Health Monitor — Version 13 (Full Script)
...
def build_ezconnect(host: str, port: str, service: Optional[str], sid: Optional[str]) -> List[str]:
    host = (host or "").strip(); port = (port or "1521").strip()
    c = []
    if service:
        svc = service.strip()
        if not svc: return []
...
def is_tns_resolution_error(err: Exception) -> bool:
    msg = str(err); return ("ORA-12154" in msg) or ("TNS:" in msg and "could not resolve" in msg.lower())

@dataclass
class DbTarget:
    name: str; dsn: str; user: Optional[str]=None; password: Optional[str]=None
    password_enc: Optional[str]=None; wallet_dir: Optional[str]=None
    mode: str="thin"; environment: str="NON-PROD"

@dataclass
class DbHealth:
    status: str; details: str; version: str=""; inst_status: str=""
    sessions_curr: int=0; sessions_limit: int=0; worst_ts_pct_used: Optional[float]=None
    host: str=""; elapsed_ms: int=0; last_full_inc_backup: Optional[datetime]=None
    last_arch_backup: Optional[datetime]=None; startup_time: Optional[datetime]=None
    ts_online: Optional[int]=None; ts_total: Optional[int]=None; db_size_gb: Optional[float]=None
    error: str=""; ts: str=field(default_factory=lambda: datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def _hydrate_target(d: Dict[str, Any]) -> DbTarget:
    pwd = d.get("password"); penc = d.get("password_enc") or (_encrypt_password(pwd) if pwd else None)
    return DbTarget(name=d.get("name",""), dsn=normalize_dsn(d.get("dsn","")), user=d.get("user"), password=_decrypt_password(penc) if penc else None, password_enc=penc, wallet_dir=d.get("wallet_dir"), mode=d.get("mode","thin"), environment=d.get("environment","NON-PROD"))

def _serialize_target(t: DbTarget) -> Dict[str, Any]:
    enc = t.password_enc or (_encrypt_password(t.password) if t.password else None)
    return {"name": t.name, "dsn": normalize_dsn(t.dsn), "user": t.user, "password_enc": enc, "wallet_dir": t.wallet_dir, "mode": t.mode, "environment": t.environment}
        ...
        try:
            ...
            if base.get("targets"):
                ...
                for t in base["targets"]:
                    ...
                    if t.get("password") or normalize_dsn(t.get("dsn","")) != t.get("dsn",""): changed = True
                    new_targets.append(_serialize_target(tt))
                if changed:
                    base["targets"] = new_targets
                    with open(CONFIG_PATH, "w", encoding="utf-8") as f: json.dump(base, f, indent=2, default=str)
            return base
...
def save_config(cfg: Dict[str, Any]):
    ts = []
    for t in cfg.get("targets", []): ts.append(_serialize_target(_hydrate_target(t) if isinstance(t, dict) else t))
...
def try_connect(user, pwd, dsn, wallet_dir=None):
    if wallet_dir: return oracledb.connect(config_dir=wallet_dir, dsn=dsn)
    if user and pwd: return oracledb.connect(user=user, password=pwd, dsn=dsn)
...
def _connect(target: DbTarget):
    ...
    if target.mode.lower() == "thick":
        ...
        if libdir:
            try: oracledb.init_oracle_client(lib_dir=libdir)
            except Exception: pass
...
@dataclass
class _RowResult:
    status_cell: str = "-"; inst_cell: str = "-"; sessions_cell: str = "-"
    worst_cell: str = "-"; last_full_cell: str = "-"; last_arch_cell: str = "-"
    version: str = "-"; startup_str: str = "-"; ts_cell: str = "-"; db_size_cell: str = "-"
    ms: int = 0; last_checked: str = "-"; check_status: str = "-"; error: str = ""
...
def check_one(target: DbTarget, timeout_sec: int = 25) -> 'DbHealth':
    ...
    try:
        with _connect(target) as conn:
            ...
            try:
                cur.execute(SQLS["sess_curr"]); sessions_curr = int(cur.fetchone()[0])
            except Exception: pass
            try:
                cur.execute(SQLS["sess_limit"]); sessions_limit = int(cur.fetchone()[0])
            except Exception: pass
            worst_pct = None
            try:
            ...
            try:
                cur.execute(SQLS["bk_data"]); r = cur.fetchone(); last_df = r[0] if r else None
            except Exception: pass
            try:
                cur.execute(SQLS["bk_arch"]); r = cur.fetchone(); last_arch = r[0] if r else None
            except Exception: pass
            elapsed_ms = int((time.time() - t0) * 1000)
            return DbHealth(status="UP", details=details, version=inst_version, inst_status=inst_status, sessions_curr=sessions_curr, sessions_limit=sessions_limit, worst_ts_pct_used=worst_pct, host=host_name, elapsed_ms=elapsed_ms, last_full_inc_backup=last_df, last_arch_backup=last_arch, startup_time=startup_time, ts_online=ts_online, ts_total=ts_total, db_size_gb=db_size_gb, error="")
    except Exception as e:
        elapsed_ms = int((time.time() - t0) * 1000)
...
class MonitorApp(ttk.Frame):
    ...
    def _install_titlebar(self, root: tk.Tk):
        ...
        self._title_frame = tk.Frame(root, bg="#ffd8a8", relief="flat", bd=0); self._title_frame.pack(side=tk.TOP, fill=tk.X)
        self._title_label = tk.Label(self._title_frame, text=APP_NAME, bg="#ffd8a8", fg="#000000", font=("Segoe UI", 10, "bold"), padx=10, pady=4, anchor="w"); self._title_label.pack(side=tk.LEFT, fill=tk.X, expand=True)
        btn_frame = tk.Frame(self._title_frame, bg="#ffd8a8"); btn_frame.pack(side=tk.RIGHT)
        def close_win(): root.destroy()
        def min_win(): root.update_idletasks(); root.overrideredirect(False); root.iconify()
        ...
        self._title_frame.bind("<Button-1>", start_move); self._title_frame.bind("<B1-Motion>", do_move)
        self._title_label.bind("<Button-1>", start_move); self._title_label.bind("<B1-Motion>", do_move)
        def map_event(e): root.overrideredirect(True)
    ...
    def _build_ui(self):
        ...
        style.configure("App.Treeview.Heading", background="#cfe8ff", foreground="#000000", font=(self._font.actual('family'), self._font.actual('size'), "bold"))
    ...
    def _add_target(self, t: DbTarget):
        ...
        t.dsn = normalize_dsn(t.dsn); self.targets.append(t); self._persist_targets()
    ...
    def _update_target(self, t: DbTarget):
        if t.password and not t.password_enc: t.password_enc = _encrypt_password(t.password)
        t.dsn = normalize_dsn(t.dsn); found = False
    ...
    def _import_json(self):
        ...
        try:
            ...
            self.cfg.update(cfg); if "email" in cfg: self.cfg["email"].update(cfg["email"] or {})
...
class DbEditor(tk.Toplevel):
    def __init__(self, parent: 'MonitorApp', target: Optional[DbTarget] = None, on_save=None):
        ...
        self.var_host = tk.StringVar(value=""); self.var_port = tk.StringVar(value="1521")
        self.var_use_service = tk.BooleanVar(value=True); self.var_sid = tk.StringVar(value=""); self.var_service = tk.StringVar(value="")
        ...
        self.row_host = ttk.Frame(frm); ttk.Label(self.row_host, text="Host:", width=22).pack(side=tk.LEFT)
        ttk.Entry(self.row_host, textvariable=self.var_host, width=50).pack(side=tk.LEFT, padx=4)

        self.row_port = ttk.Frame(frm); ttk.Label(self.row_port, text="Port:", width=22).pack(side=tk.LEFT)
        ttk.Entry(self.row_port, textvariable=self.var_port, width=12).pack(side=tk.LEFT, padx=4)

        self.row_sid_serv = ttk.Frame(frm); ttk.Label(self.row_sid_serv, text="Connect Using:", width=22).pack(side=tk.LEFT)
        ...
        self.row_service = ttk.Frame(frm); ttk.Label(self.row_service, text="Service Name:", width=22).pack(side=tk.LEFT)
        ttk.Entry(self.row_service, textvariable=self.var_service, width=40).pack(side=tk.LEFT, padx=4)

        self.row_sid = ttk.Frame(frm); ttk.Label(self.row_sid, text="SID:", width=22).pack(side=tk.LEFT)
        ...
        def row_entry(lbl, var, show=None, browse=False):
            ...
            if browse: ttk.Button(rr, text="...", width=3, command=lambda v=var: self._pick_dir(v)).pack(side=tk.LEFT)
        ...
        self._toggle_conn_type(); self._toggle_sid_service()
    ...
    def _toggle_conn_type(self):
        ...
        if tns:
            ...
            self.row_host.pack_forget(); self.row_port.pack_forget(); self.row_sid_serv.pack_forget()
            self.row_service.pack_forget(); self.row_sid.pack_forget()
        else:
            self.var_mode.set("thin")
            self.row_tns.pack_forget(); self.row_host.pack(fill=tk.X, pady=4)
            self.row_port.pack(fill=tk.X, pady=4); self.row_sid_serv.pack(fill=tk.X, pady=4)

    def _toggle_sid_service(self):
        if self.var_use_service.get():
            self.row_service.pack(fill=tk.X, pady=4); self.row_sid.pack_forget()
        else:
            self.row_service.pack_forget(); self.row_sid.pack(fill=tk.X, pady=4)
    ...
    def _build_dsn_candidates(self) -> List[str]:
        if self.var_conn_type.get() == "jdbc":
            ...
            if self.var_use_service.get():
                service = self.var_service.get().strip(); return build_ezconnect(host, port, service=service, sid=None)
            else:
                sid = self.var_sid.get().strip(); return build_ezconnect(host, port, service=None, sid=sid)
        dsn = self.var_dsn.get().strip(); return [normalize_dsn(dsn), f"//{normalize_dsn(dsn)}"]
    ...
    def _test_connection(self):
        ...
        for dsn in dsn_candidates:
            try:
                if wallet: conn = oracledb.connect(config_dir=wallet, dsn=dsn)
                elif user and pwd: conn = oracledb.connect(user=user, password=pwd, dsn=dsn)
                else: conn = oracledb.connect(dsn=dsn)
                try:
                    cur = conn.cursor(); cur.execute("SELECT 'OK' FROM dual"); res = cur.fetchone()[0]
                finally: conn.close()
                messagebox.showinfo(APP_NAME, f"Connection successful via '{dsn}' ({res})."); return
            except Exception as e:
                last_err = e; continue
    ...
    def _save(self):
        ...
        if self.var_conn_type.get() == "jdbc":
            candidates = self._build_dsn_candidates(); dsn = candidates[0] if candidates else ""; mode = "thin"
        else:
            dsn = normalize_dsn(self.var_dsn.get().strip()); mode = "thick"
        pwd = self.var_pwd.get().strip() or None
        t = DbTarget(name=name, dsn=dsn, user=self.var_user.get().strip() or None, password=pwd, password_enc=_encrypt_password(pwd) if pwd else None, wallet_dir=self.var_wallet.get().strip() or None, mode=mode, environment=self.var_env.get().strip() or "NON-PROD")
