#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Oracle DB Health Monitor — Version 15
# See chat message for the list of fixes.
...
def looks_like_ezconnect(dsn: str) -> bool:
    ...
    return ("/" in s) or ("?sid=" in s) or (":" in s and s.count(":") >= 1 and "/" in s)
...
class DbEditor(tk.Toplevel):
    def __init__(self, parent: 'MonitorApp', target: Optional[DbTarget] = None, on_save=None):
        ...
        tns_pwd = _decrypt_password(target.password_enc) if (target and target.password_enc) else None
        self.var_pwd = tk.StringVar(value=tns_pwd or "")
    ...
    def _test_connection(self):
        ...
        try:
            if self.var_conn_type.get() == "tns":
                ...
                if not looks_like_ezconnect(dsn): messagebox.showerror(APP_NAME, "Invalid EZCONNECT."); return
    ...
    def _save(self):
        ...
        if self.var_conn_type.get() == "tns":
            ...
            if not (host and port and idval): messagebox.showerror(APP_NAME, "Enter Host, Port and SID/ServiceName."); return
            dsn = ezconnect_service(host, port, idval) if self.var_use_service.get() else ezconnect_sid(host, port, idval)
            t = DbTarget(name=name, dsn=dsn, user=None, password_enc=None, mode="thin", environment=self.var_env.get().strip() or "NON-PROD")
