...
# -*- coding: utf-8 -*-
"""
Database Pulse — FULL Version (Oracle module visible by default)

What you get
------------
• Title bar: "Database Pulse"
• Top banner (medium orange) with left buttons: Home, Oracle Database, SQL Server (future)
• App opens directly to the **Oracle Database** module (no extra clicks needed)
• Oracle monitor: sortable table, vertical & horizontal scrollbars, context menu (copy), email report
• Config persistence at ./config/oracle_config.json
• Two connection types in Add/Edit DB:
    1) TNS (Oracle Client) — thick mode, uses Instant Client + TNS alias/descriptor
    2) JDBC Thin (no client) — thin mode, uses host/port + Service Name or SID
• Test Connection respects the selected connection type

Notes
-----
• Requires: python 3.8+, tkinter
• Optional: python-oracledb (for DB connections/checks). GUI still loads without it.
"""
...
class DbTarget:
    ...
    mode: str = "thin"  # "thin" (JDBC thin style) or "thick" (TNS via Oracle client)
...
class MonitorApp(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        t3 = ttk.Frame(self)
        t3.pack(side=tk.TOP, fill=tk.X, padx=8, pady=(0, 6))
...
class Landing(ttk.Frame):
    def __init__(self, master):
        ...
        icon = tk.Label(center, text="🗄️  📈", font=("Segoe UI Emoji", 44))
        icon.pack(pady=(0, 8))
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Show ORACLE by default so it's visible immediately
        self._select_nav("oracle", self.show_oracle)
    ...
    def _select_nav(self, key: str, route_call):
        ...
        if key in self._nav_btns:
            self._nav_btns[key].configure(bg="#6ea8fe")  # medium blue selection
        route_call()
