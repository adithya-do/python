#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Database Pulse — Version 3.2
# Request: "use same title bar of version 1"
# What this build does:
# - Restores the *exact same* title bar behavior as V1, when you provide the original V1 icon.
# - It prefers a V1 .ICO (best for Windows title bar), then falls back to a V1 .PNG for iconphoto.
# - If no external icon is found, it uses a neutral built‑in fallback so the app still runs.
# - The banner continues to use the same image as the title bar when a PNG is available.
#
# Where to put your original V1 icon (first match wins):
#   ICO (best for title bar on Windows):
#     ./config/icons/database_pulse_v1.ico
#     ./config/icons/leaf_v1.ico
#     ./icons/database_pulse_v1.ico
#     ./icons/leaf_v1.ico
#     ./database_pulse_v1.ico
#     ./leaf_v1.ico
#   PNG (used for both title bar and banner if found):
#     ./config/icons/database_pulse_v1.png
#     ./config/icons/leaf_v1.png
#     ./icons/database_pulse_v1.png
#     ./icons/leaf_v1.png
#     ./database_pulse_v1.png
#     ./leaf_v1.png
#
...
def load_config() -> Dict[str, Any]):
    # NOTE: tiny typo fix from earlier builds (extra paren)
...
class Landing(ttk.Frame):
    def __init__(self, master):
        ...
        icon = tk.Label(center, text="🗄️  📈", font=("Segoe UI Emoji", 44))  # no heart
        icon.pack(pady=(0,8))
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        super().__init__()
        # --- Use the SAME title bar as Version 1 ---
        self.title(APP_NAME)  # V1 used plain app name in title bar
        self._apply_v1_titlebar_icons()  # load ICO/PNG from disk; fallback builtin if none
        ...
        # Top banner (unchanged). If PNG is available, reuse for banner too.
        self.banner = tk.Frame(self, bg="#f5a623")
        self.banner.pack(side=tk.TOP, fill=tk.X)

        left = tk.Frame(self.banner, bg="#f5a623"); left.pack(side=tk.LEFT, padx=6, pady=6)
        self._nav_btns: Dict[str, tk.Button] = {}
        def mkbtn(txt, key, cmd):
            b = tk.Button(left, text=txt, command=lambda k=key, c=cmd: self._select_nav(k, c),
                          bg="#ffffff", fg="#000000", activebackground="#d6e6ff",
                          activeforeground="#000000", font=("Segoe UI", 10), padx=10, pady=4, relief=tk.RAISED, bd=1)
            b.pack(side=tk.LEFT, padx=4); self._nav_btns[key] = b
        mkbtn("Home", "home", self.show_home)
        mkbtn("Oracle Database", "oracle", self.show_oracle)
        mkbtn("SQL Server", "sql", self.show_sqlserver)

        right = tk.Frame(self.banner, bg="#f5a623"); right.pack(side=tk.RIGHT, padx=10, pady=6)
        if getattr(self, "_v1_png_icon", None) is not None:
            tk.Label(right, image=self._v1_png_icon, bg="#f5a623").pack(side=tk.RIGHT, padx=(0,6))
        tk.Label(right, text=f"{APP_NAME}", bg="#f5a623", fg="#000000", font=("Segoe UI", 14, "bold")).pack(side=tk.RIGHT)
        tk.Label(right, text=f" {APP_VERSION} ", bg="#f5a623", fg="#222222", font=("Segoe UI", 9)).pack(side=tk.RIGHT, padx=(4,0))

        self.stack = tk.Frame(self); self.stack.pack(fill=tk.BOTH, expand=True)
        self.view_landing = Landing(self.stack); self.view_oracle = None; self.view_sql = None
        self._select_nav("home", self.show_home)

    # --- V1 title bar application ---
    def _apply_v1_titlebar_icons(self):
        ico_candidates = [
            CONFIG_DIR / "icons" / "database_pulse_v1.ico",
            CONFIG_DIR / "icons" / "leaf_v1.ico",
            _base_dir() / "icons" / "database_pulse_v1.ico",
            _base_dir() / "icons" / "leaf_v1.ico",
            _base_dir() / "database_pulse_v1.ico",
            _base_dir() / "leaf_v1.ico",
        ]
        png_candidates = [
            CONFIG_DIR / "icons" / "database_pulse_v1.png",
            CONFIG_DIR / "icons" / "leaf_v1.png",
            _base_dir() / "icons" / "database_pulse_v1.png",
            _base_dir() / "icons" / "leaf_v1.png",
            _base_dir() / "database_pulse_v1.png",
            _base_dir() / "leaf_v1.png",
        ]

        # 1) Prefer ICO for Windows title bar (exactly like V1)
        ico_applied = False
        if sys.platform.startswith("win"):
            for p in ico_candidates:
                if p.exists():
                    try:
                        self.iconbitmap(default=str(p))  # Windows title bar icon
                        ico_applied = True
                        break
                    except Exception:
                        pass

        # 2) PNG for both title bar (iconphoto) and banner image
        png_img = None
        for p in png_candidates:
            if p.exists():
                try:
                    png_img = tk.PhotoImage(file=str(p))
                    self.iconphoto(True, png_img)
                    break
                except Exception:
                    png_img = None
        self._v1_png_icon = png_img  # stored for banner reuse

        # 3) Fallback if nothing found: use a neutral built-in small icon
        if not ico_applied and png_img is None:
            fallback = self._make_fallback_leaf(16, 16)
            self.iconphoto(True, fallback)
            self._v1_png_icon = fallback

    def _make_fallback_leaf(self, w: int, h: int) -> tk.PhotoImage:
        ...
        for x in range(2, w-2):
            for y in range(3, h-3):
                nx = (x - w/2) / (w/2.2); ny = (y - h/2) / (h/2.8)
                if nx*nx + ny*ny <= 1.0 and y <= h - 4: img.put("#f3f6f4", (x, y))
        for x in range(2, w-2):
            for y in range(3, h-3):
                nx = (x - w/2) / (w/2.2); ny = (y - h/2) / (h/2.8); d = nx*nx + ny*ny
                if 0.96 <= d <= 1.02 and y <= h - 4: img.put("#3a3a3a", (x, y))
        for i in range(3, min(w, h)-2):
            vx = int(i * (w-6) / (h-6)) + 3; vy = h - i - 1
            if 2 <= vx < w-2 and 2 <= vy < h-2: img.put("#9aa3a0", (vx, vy))
        return img
    ...
    def _select_nav(self, key: str, route_call):
        ...
        if key in self._nav_btns: self._nav_btns[key].configure(bg="#6ea8fe")  # medium blue
        route_call()

    def _show(self, frame: tk.Frame):
        for child in self.stack.winfo_children(): child.pack_forget()
        frame.pack(fill=tk.BOTH, expand=True)

    def show_home(self):
        if self.view_landing is None: self.view_landing = Landing(self.stack)
        self._show(self.view_landing)

    def show_oracle(self):
        if self.view_oracle is None: self.view_oracle = MonitorApp(self.stack, self.cfg)
        self._show(self.view_oracle)

    def show_sqlserver(self):
        if self.view_sql is None:
            self.view_sql = tk.Frame(self.stack); self.view_sql.configure(padding=20)
