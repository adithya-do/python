#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Database Pulse — Version 19 (Stable base: v18)
# Changes:
# 1) Window title changed to "Database Pulse".
# 2) Top banner (medium orange) with two buttons: "Oracle Database" and "SQL Server".
#    - "Oracle Database" swaps in the Oracle monitoring UI (existing app).
#    - "SQL Server" shows a placeholder panel for future development.
# 3) Default view is a blank landing pane with big centered title
#    "Database Pulse" + small subtitle "A Database Health Check Tool" and a pulse icon.
#
...
# ---------------- Oracle Monitor UI (existing app) ----------------
class MonitorApp(ttk.Frame):
    ...
    # ... (All MonitorApp methods remain identical to v18; omitted for brevity here)
    # For space, we include the same implementations as in your stable v18 build.
    # ---- START v18 method bodies ----
    ...
    def _apply_result(self, name: str, target: DbTarget, h: DbHealth):
        ...
        self.tree.item(name, values=vals)
        self.parent().last_health = getattr(self, "last_health", {})
    ...
    def _clear_selected_row(self):
        sel = self.tree.selection()
        if not sel: messagebox.showinfo("Info", "Select a row to clear."); return
    ...
    def _edit_selected(self):
        sel = self.tree.selection()
        if not sel: messagebox.showinfo("Info", "Select a row to edit."); return
        name = sel[0]; t = next((x for x in self.targets if x.name == name), None)
        if not t: messagebox.showerror("Error", "Target not found."); return
    ...
    def _add_target(self, t: DbTarget):
        if any(x.name == t.name for x in self.targets):
            messagebox.showerror("Error", "A target with this name already exists."); return
    ...
    def _import_json(self):
        ...
        try:
            ...
            messagebox.showinfo("Info", "Imported configuration.")
        except Exception as e:
    ...
    def _export_json(self):
        ...
        try:
            ...
            messagebox.showinfo("Info", "Exported configuration.")
        except Exception as e:
    ...
    def _save_mail_settings(self):
        ...
        save_config(self.cfg); messagebox.showinfo("Info", "Mail settings saved.")
    ...
    def _email_report(self):
        ...
        if not (server and from_addr and to_addrs):
            messagebox.showerror("Error", "Set SMTP server, From, and To addresses first."); return
        ...
        try:
            ...
            messagebox.showinfo("Info", "Email report sent.")
        except Exception as e:
...
class Landing(ttk.Frame):
    def __init__(self, master):
        ...
        title = tk.Label(center, text="Database Pulse", font=("Segoe UI", 28, "bold"))
        subtitle = tk.Label(center, text="A Database Health Check Tool  ❤️‍🩹", font=("Segoe UI", 14))
        # simple db cylinder + pulse using unicode
        icon = tk.Label(center, text="🗄️  📈", font=("Segoe UI Emoji", 36))
        icon.pack(pady=(0,6))
        title.pack()
...
class RouterApp(tk.Tk):
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Top banner
        self.banner = tk.Frame(self, bg="#f5a623")  # medium orange
        self.banner.pack(side=tk.TOP, fill=tk.X)
        # Left title label inside banner for aesthetics
        tk.Label(self.banner, text="  Database Pulse", bg="#f5a623", fg="#000000",
                 font=("Segoe UI", 14, "bold")).pack(side=tk.LEFT, padx=6, pady=6)

        # Buttons on banner
        btn_style = {"bg":"#ffffff", "fg":"#000000", "activebackground":"#ffe3ba", "activeforeground":"#000000"}
        tk.Button(self.banner, text="Oracle Database", command=self.show_oracle, **btn_style).pack(side=tk.RIGHT, padx=6, pady=6)
        tk.Button(self.banner, text="SQL Server", command=self.show_sqlserver, **btn_style).pack(side=tk.RIGHT, padx=6, pady=6)
